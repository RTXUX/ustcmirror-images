...
def encode_tag(tag):
    return strip_prefix(tag, 'ghcr.io/ustcmirror-test/').replace(':', '.')
...
def get_dest_image(img, f):
    ...
    if tag:
        return 'ghcr.io/ustcmirror-test/{}:{}'.format(img, tag[1:])
    else:
        return 'ghcr.io/ustcmirror-test/{}:latest'.format(img)
...
def main():
    ...
    with Builder() as b:
        for dst, base in imgs.items():
            if base.startswith('ghcr.io/ustcmirror-test'):
                b.add(dst, base)
            else:
                b.add(dst, '')
        b.finish()
