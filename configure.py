...
def encode_tag(tag):
    return strip_prefix(tag, 'ghcr.io/rtxux/ustcmirror-').replace(':', '.')
...
def get_dest_image(img, f):
    ...
    if tag:
        return 'ghcr.io/rtxux/ustcmirror-{}:{}'.format(img, tag[1:])
    else:
        return 'ghcr.io/rtxux/ustcmirror-{}:latest'.format(img)
...
def main():
    ...
    with Builder() as b:
        for dst, base in imgs.items():
            if base.startswith('ghcr.io/rtxux/ustcmirror-'):
                b.add(dst, base)
            else:
                b.add(dst, '')
        b.finish()
